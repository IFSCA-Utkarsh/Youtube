...
from .config import CHANNEL_SECTIONS, COOKIES_FILE, THEME_CSS
...
class YouTubeStudioApp:
    ...
    def _build_cookie_banner(self):
        """
        Small optional control: lets the user upload a cookies.txt exported
        from a logged-in browser session. This fixes YouTube's "Sign in to
        confirm you're not a bot" error, which cloud IPs like Colab's
        frequently trigger regardless of which video is requested.
        """
        import os

        has_cookies = os.path.isfile(COOKIES_FILE)
        self.cookie_status = widgets.HTML(
            f"<span style='color:#81c784;'>🍪 cookies.txt loaded — bot-check bypass active.</span>"
            if has_cookies
            else "<span style='color:#9aa0b4;'>🍪 No cookies.txt loaded (optional — only needed if you "
                 "hit a \"Sign in to confirm you're not a bot\" error).</span>"
        )
        self.cookie_upload = widgets.FileUpload(
            description="Upload cookies.txt", accept=".txt", multiple=False,
            layout=widgets.Layout(width="200px"),
        )
        self.cookie_upload.observe(self._on_cookie_upload, names="value")
        self.cookie_banner = widgets.HBox([self.cookie_upload, self.cookie_status])

    def _on_cookie_upload(self, change):
        uploaded = change["new"]
        if not uploaded:
            return
        item = uploaded[0] if isinstance(uploaded, (list, tuple)) else next(iter(uploaded.values()))
        content = item["content"]
        try:
            with open(COOKIES_FILE, "wb") as f:
                f.write(bytes(content))
            self.cookie_status.value = (
                f"<span style='color:#81c784;'>🍪 cookies.txt saved — bot-check bypass active.</span>"
            )
        except Exception as exc:
            self.cookie_status.value = f"<span style='color:#e57373;'>⚠️ Could not save cookies.txt: {exc}</span>"
